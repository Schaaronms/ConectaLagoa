...
const generateToken = (id, tipo) => {
  return jwt.sign({ id, tipo }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRE || '7d' });
};
...
const registroCandidato = async (req, res) => {
  try {
    ...
    const result = await db.pool.query(
      `INSERT INTO candidatos (nome_completo, email, senha, telefone, cidade, estado) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`,
    ...
    res.status(201).json({ success: true, message: 'Candidato cadastrado com sucesso', token, user: { id: candidatoId, nome: nome_completo, email, tipo: 'candidato' } });
  } catch (error) {
...
const registroEmpresa = async (req, res) => {
  try {
    ...
    const senhaHash = await bcrypt.hash(senha, 10);
    const result = await db.pool.query(
      `INSERT INTO empresas (nome, email, senha, cnpj, telefone, cidade, estado) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`,
    ...
    res.status(201).json({ success: true, message: 'Empresa cadastrada com sucesso', token, user: { id: empresaId, nome, email, tipo: 'empresa' } });
  } catch (error) {
...
const login = async (req, res) => {
  try {
    ...
    if (!['candidato', 'empresa'].includes(tipo)) return res.status(400).json({ success: false, message: 'Tipo inválido' });
    ...
    res.json({ success: true, message: 'Login bem-sucedido', token, user: { id: user.id, nome: user[nomeField], email: user.email, tipo } });
  } catch (error) {
  ...
  try {
    ...
    await enviarEmail({
      ...
      html: `
        ...
        <a href="${link}" style="background:#007bff;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;">Redefinir senha</a>
