...
function ModalAvaliacao({ open, onClose, onSaved, colaboradores }) {
  ...
  const handleSalvar = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const BASE  = process.env.REACT_APP_API_URL || 'https://conectalagoa.onrender.com/api';
      const res   = await fetch(`${BASE}/avaliacoes`, { method:'POST', headers:{'Content-Type':'application/json','Authorization':`Bearer ${token}`}, body:JSON.stringify(form) });
      if (res.ok) { onSaved(); onClose(); }
      ...
      <div style={{ background:V.surface, border:`1px solid ${V.border}`, borderRadius:16, padding:28, width:560, maxWidth:'100%', maxHeight:'90vh', overflowY:'auto', position:'relative' }}>
        ...
        <div style={{ display:'flex', gap:12, marginBottom:14 }}>
          ...
          <div style={{ flex:1 }}>
            ...
            <input value={form.periodo} onChange={set('periodo')} placeholder="ex: 2025-T1" style={inp} onFocus={e=>e.target.style.borderColor=V.accent} onBlur={e=>e.target.style.borderColor=V.border}/>
          </div>
        </div>
        <div style={{ marginBottom:18 }}>
          <label style={lbl}>Avaliador</label>
          <input value={form.avaliador_nome} onChange={set('avaliador_nome')} placeholder="Nome do gestor" style={inp} onFocus={e=>e.target.style.borderColor=V.accent} onBlur={e=>e.target.style.borderColor=V.border}/>
        </div>
        ...
        <div style={{ marginBottom:20 }}>
          ...
          <textarea value={form.observacao} onChange={set('observacao')} rows={3} placeholder="Pontos fortes, áreas de melhoria..." style={{ ...inp, resize:'vertical' }} onFocus={e=>e.target.style.borderColor=V.accent} onBlur={e=>e.target.style.borderColor=V.border}/>
        </div>
        <div style={{ display:'flex', gap:10, justifyContent:'flex-end' }}>
          <button onClick={onClose} style={{ background:'none', border:`1px solid ${V.border}`, color:V.muted2, padding:'8px 18px', borderRadius:8, cursor:'pointer', fontSize:12 }}>Cancelar</button>
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      const [avRes, mdRes] = await Promise.all([
        fetch(`${BASE}/avaliacoes${p}`,           { headers:{'Authorization':`Bearer ${token}`} }),
...
function ModalCargo({ open, onClose, onSaved, cargo = null }) {
  ...
  const handleSalvar = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const BASE  = process.env.REACT_APP_API_URL || 'https://conectalagoa.onrender.com/api';
      const res = await fetch(cargo ? `${BASE}/cargos/${cargo.id}` : `${BASE}/cargos`, {
        method: cargo ? 'PUT' : 'POST', headers:{'Content-Type':'application/json','Authorization':`Bearer ${token}`}, body:JSON.stringify(form),
      });
      if (res.ok) { onSaved(); onClose(); }
      ...
      <div style={{ background:V.surface, border:`1px solid ${V.border}`, borderRadius:16, padding:28, width:500, maxWidth:'100%' }}>
        ...
        <div style={{ display:'flex', gap:12, marginBottom:14 }}>
          <div style={{ flex:2 }}>
            ...
            <input value={form.nome} onChange={set('nome')} placeholder="ex: Analista de Vendas" style={inp} onFocus={e=>e.target.style.borderColor=V.accent} onBlur={e=>e.target.style.borderColor=V.border}/>
          </div>
          <div style={{ flex:1 }}>
            <label style={lbl}>Departamento</label>
            <select value={form.departamento} onChange={set('departamento')} style={inp}>
...
function SubCargosSalarios() {
  ...
  useEffect(() => { fetchAll(); }, []);
  const handleDelete = async (id) => { await fetch(`${BASE}/cargos/${id}`,{method:'DELETE',headers:{'Authorization':`Bearer ${token}`}}); fetchAll(); };
...
function PanelColaboradores() {
  ...
  const handleDelete  = async (id) => {
    setDeletandoId(id);
    try { await fetch(`${BASE}/colaboradores/${id}`,{method:'DELETE',headers:{'Authorization':`Bearer ${token}`}}); fetchLista(); loadColaboradores(); }
  ...
  const SUB_TABS = [{id:'lista',label:'👥 Lista'},{id:'avaliacao',label:'📋 Avaliação de Desempenho'},{id:'cargos',label:'💼 Cargos e Salários'}];
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', gap:4, marginBottom:24, borderBottom:`1px solid ${V.border}`, overflowX:'auto' }}>
        {SUB_TABS.map(t => (
          <button key={t.id} onClick={() => setSubTab(t.id)} style={{ padding:'10px 22px', fontSize:13, fontWeight:500, cursor:'pointer', border:'none', background:'none', whiteSpace:'nowrap', borderBottom:`2px solid ${subTab===t.id?V.accent:'transparent'}`, color:subTab===t.id?V.accent:V.muted, transition:'all 0.2s', fontFamily:"'DM Sans',sans-serif" }}>{t.label}</button>
        ))}
      </div>
        ...
        <div>
                    ...
                    {lista.map((c, i) => {
                      ...
                      return (
                        <tr key={c.id} style={{ borderBottom:`1px solid rgba(226,232,244,0.5)`, transition:'background 0.15s', animation:`fadeUp 0.3s ease ${i*0.03}s both` }} onMouseEnter={e=>e.currentTarget.style.background='rgba(26,58,143,0.03)'} onMouseLeave={e=>e.currentTarget.style.background=''}>
                          ...
                          <td style={{ padding:'12px 14px' }}><span style={{ fontSize:10, padding:'3px 9px', borderRadius:20, fontWeight:600, background:c.status==='ativo'?'rgba(16,185,129,0.12)':'rgba(107,114,128,0.12)', color:statusColor(c.status) }}>{c.status==='ativo'?'● Ativo':'○ Desligado'}</span></td>
                          <td style={{ padding:'12px 14px' }}>
                            <div style={{ display:'flex', gap:6 }}>
                              <button title="Editar" onClick={() => { setEditando(c); setModalForm(true); }} style={{ background:'none', border:`1px solid ${V.border}`, color:V.muted2, width:28, height:28, borderRadius:6, cursor:'pointer', fontSize:13, display:'flex', alignItems:'center', justifyContent:'center', transition:'all 0.15s' }} onMouseEnter={e=>{e.currentTarget.style.borderColor=V.accent;e.currentTarget.style.color=V.accent;}} onMouseLeave={e=>{e.currentTarget.style.borderColor=V.border;e.currentTarget.style.color=V.muted2;}}>✏</button>
                              {c.status==='ativo' && <button title="Desligar" onClick={() => setDesligando(c)} style={{ background:'none', border:`1px solid ${V.border}`, color:V.muted2, width:28, height:28, borderRadius:6, cursor:'pointer', fontSize:13, display:'flex', alignItems:'center', justifyContent:'center', transition:'all 0.15s' }} onMouseEnter={e=>{e.currentTarget.style.borderColor=V.orange;e.currentTarget.style.color=V.orange;}} onMouseLeave={e=>{e.currentTarget.style.borderColor=V.border;e.currentTarget.style.color=V.muted2;}}>🚪</button>}
                              {confirmDelete===c.id ? (
                                ...
                                <button title="Excluir" onClick={() => setConfirmDelete(c.id)} style={{ background:'none', border:`1px solid ${V.border}`, color:V.muted2, width:28, height:28, borderRadius:6, cursor:'pointer', fontSize:13, display:'flex', alignItems:'center', justifyContent:'center', transition:'all 0.15s' }} onMouseEnter={e=>{e.currentTarget.style.borderColor=V.red;e.currentTarget.style.color=V.red;}} onMouseLeave={e=>{e.currentTarget.style.borderColor=V.border;e.currentTarget.style.color=V.muted2;}}>🗑</button>
                              )}
                            </div>
                          </td>
                        </tr>
          ...
          {pages > 1 && (
            <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:8, marginTop:16 }}>
              <button onClick={() => { setPage(p=>Math.max(1,p-1)); fetchLista(Math.max(1,page-1)); }} disabled={page===1} style={{ background:'none', border:`1px solid ${V.border}`, color:page===1?V.muted2:V.text, padding:'6px 14px', borderRadius:8, cursor:page===1?'default':'pointer', fontSize:12 }}>‹ Anterior</button>
              <span style={{ fontSize:12, color:V.muted }}>Página {page} de {pages} · {total} registros</span>
              <button onClick={() => { setPage(p=>Math.min(pages,p+1)); fetchLista(Math.min(pages,page+1)); }} disabled={page===pages} style={{ background:'none', border:`1px solid ${V.border}`, color:page===pages?V.muted2:V.text, padding:'6px 14px', borderRadius:8, cursor:page===pages?'default':'pointer', fontSize:12 }}>Próxima ›</button>
            </div>
          )}
