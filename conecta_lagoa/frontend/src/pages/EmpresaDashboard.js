...
function RadarChart({ data, size = 220 }) {
  ...
  const polyPoints = (frac) =>
    data.map((_, i) => { const p = point(i, r * frac); return `${p.x},${p.y}`; }).join(' ');
  const dataPoints = data
    .map((d, i) => { const p = point(i, r * (d.value / 10)); return `${p.x},${p.y}`; })
    .join(' ');
  return (
    <svg width={size} height={size} style={{ overflow:'visible' }}>
      {gridLevels.map((f, li) => (
        <polygon key={li} points={polyPoints(f)} fill="none" stroke={V.border} strokeWidth="1"/>
      ))}
      {data.map((_, i) => {
        const p = point(i, r);
        return <line key={i} x1={cx} y1={cy} x2={p.x} y2={p.y} stroke={V.border} strokeWidth="1"/>;
      })}
      ...
      {data.map((d, i) => {
        ...
        return (
          <text key={i} x={lp.x} y={lp.y} textAnchor="middle" dominantBaseline="middle"
            fontSize="10" fill={V.muted} fontFamily="'DM Sans',sans-serif">
            {d.label}
            <tspan x={lp.x} dy="12" fontSize="11" fontWeight="700" fill={V.text}>{d.value}</tspan>
          </text>
        );
      })}
      {data.map((d, i) => {
        const p = point(i, r * (d.value / 10));
        return <circle key={i} cx={p.x} cy={p.y} r="4" fill={V.accent}/>;
      })}
    </svg>
  );
}
...
function ModalAvaliacao({ open, onClose, onSaved, colaboradores }) {
  const EMPTY = { colaborador_id:'', periodo:'', avaliador_nome:'',
    comunicacao:'', trabalho_equipe:'', iniciativa:'', lideranca:'', organizacao:'', observacao:'' };
  const [form, setForm]     = useState(EMPTY);
  const [saving, setSaving] = useState(false);
  const [toast, setToast]   = useState('');
  ...
  const nota_geral = () => {
    const vals = ['comunicacao','trabalho_equipe','iniciativa','lideranca','organizacao']
      .map(k => parseFloat(form[k])).filter(v => !isNaN(v) && v >= 0 && v <= 10);
  ...
  const handleSalvar = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const BASE  = process.env.REACT_APP_API_URL || 'https://conectalagoa.onrender.com/api';
      const res   = await fetch(`${BASE}/avaliacoes`, {
        method: 'POST',
        headers: { 'Content-Type':'application/json', 'Authorization':`Bearer ${token}` },
        body: JSON.stringify(form),
      });
  ...
  return (
    <div onClick={e=>{ if(e.target===e.currentTarget) onClose(); }}
      ...
      <div style={{ background:V.surface, border:`1px solid ${V.border}`, borderRadius:16, padding:28, width:560, maxWidth:'100%', maxHeight:'90vh', overflowY:'auto', position:'relative' }}>
        {toast && (
          <div style={{ position:'absolute', top:16, left:'50%', transform:'translateX(-50%)', background:V.red, color:'white', padding:'6px 18px', borderRadius:20, fontSize:12, fontWeight:500, whiteSpace:'nowrap', zIndex:10 }}>{toast}</div>
        )}
        ...
        <div style={{ display:'flex', gap:12, marginBottom:14 }}>
          ...
          <div style={{ flex:1 }}>
            ...
            <input value={form.periodo} onChange={set('periodo')} placeholder="ex: 2025-T1" style={inp}
              onFocus={e=>e.target.style.borderColor=V.accent} onBlur={e=>e.target.style.borderColor=V.border}/>
          </div>
        </div>
        <div style={{ marginBottom:18 }}>
          <label style={lbl}>Avaliador</label>
          <input value={form.avaliador_nome} onChange={set('avaliador_nome')} placeholder="Nome do gestor avaliador" style={inp}
            onFocus={e=>e.target.style.borderColor=V.accent} onBlur={e=>e.target.style.borderColor=V.border}/>
        </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            {COMPETENCIAS.map(({ key, label }) => (
              <div key={key}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                  <input type="number" min="0" max="10" step="0.1" value={form[key]} onChange={set(key)} placeholder="0–10"
                    style={{ ...inp, width:80 }}
                    onFocus={e=>e.target.style.borderColor=V.accent} onBlur={e=>e.target.style.borderColor=V.border}/>
                  <div style={{ flex:1, height:6, background:V.border, borderRadius:3, overflow:'hidden' }}>
                    <div style={{ height:'100%', borderRadius:3, width:`${Math.min((parseFloat(form[key])||0)/10*100,100)}%`,
                      background:(parseFloat(form[key])||0)>=8?V.green:(parseFloat(form[key])||0)>=6?V.orange:V.red, transition:'width 0.3s' }}/>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div style={{ marginBottom:20 }}>
          ...
          <textarea value={form.observacao} onChange={set('observacao')} rows={3}
            placeholder="Pontos fortes, áreas de melhoria, metas..."
            style={{ ...inp, resize:'vertical' }}
            onFocus={e=>e.target.style.borderColor=V.accent} onBlur={e=>e.target.style.borderColor=V.border}/>
        </div>
        <div style={{ display:'flex', gap:10, justifyContent:'flex-end' }}>
          <button onClick={onClose} style={{ background:'none', border:`1px solid ${V.border}`, color:V.muted2, padding:'8px 18px', borderRadius:8, cursor:'pointer', fontSize:12 }}>Cancelar</button>
          <button onClick={handleSalvar} disabled={saving}
            style={{ background:saving?V.muted2:V.accent, border:'none', color:'white', padding:'9px 22px', borderRadius:8, cursor:'pointer', fontSize:13, fontWeight:600 }}>
...
// ─── SUBABA: AVALIAÇÃO DE DESEMPENHO ─────────────────────────────
function SubAvaliacoes({ colaboradores }) {
  const [avaliacoes, setAvaliacoes] = useState([]);
  const [media, setMedia]           = useState(null);
  const [loading, setLoading]       = useState(true);
  const [modalOpen, setModalOpen]   = useState(false);
  const [filtColaborador, setFiltColaborador] = useState('');
  ...
  const fetchAll = async () => {
    setLoading(true);
    try {
      const params = filtColaborador ? `?colaborador_id=${filtColaborador}` : '';
      const [avRes, mdRes] = await Promise.all([
        fetch(`${BASE}/avaliacoes${params}`, { headers:{ 'Authorization':`Bearer ${token}` } }),
        fetch(`${BASE}/avaliacoes/media-equipe`, { headers:{ 'Authorization':`Bearer ${token}` } }),
      ]);
  ...
  useEffect(() => { fetchAll(); }, [filtColaborador]);

  const COMPETENCIAS = [
    { key:'comunicacao',     label:'Comunicação'   },
    { key:'trabalho_equipe', label:'Trab. Equipe'  },
    { key:'iniciativa',      label:'Iniciativa'    },
    { key:'lideranca',       label:'Liderança'     },
    { key:'organizacao',     label:'Organização'   },
  ];

  const radarData = COMPETENCIAS.map(c => ({ label:c.label, value:parseFloat(media?.[c.key]||0) }));

  const handleDelete = async (id) => {
    await fetch(`${BASE}/avaliacoes/${id}`, { method:'DELETE', headers:{ 'Authorization':`Bearer ${token}` } });
    fetchAll();
  };
    ...
    <div>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20, flexWrap:'wrap', gap:12 }}>
        ...
        <div style={{ display:'flex', gap:8, alignItems:'center' }}>
          <select value={filtColaborador} onChange={e=>setFiltColaborador(e.target.value)}
            style={{ background:V.surface2, border:`1px solid ${V.border}`, borderRadius:8, padding:'8px 12px', color:V.text, fontSize:12, outline:'none', cursor:'pointer' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'auto 1fr', gap:16, marginBottom:20 }}>
        <div style={{ background:V.surface, border:`1px solid ${V.border}`, borderRadius:14, padding:24, display:'flex', flexDirection:'column', alignItems:'center' }}>
          ...
          {media
            ...
            : <div style={{ width:220, height:220, display:'flex', alignItems:'center', justifyContent:'center', color:V.muted, fontSize:12 }}>Sem dados ainda</div>
          }
          {media && (
            <div style={{ marginTop:12, textAlign:'center' }}>
              <div style={{ fontSize:11, color:V.muted }}>Nota Geral Média</div>
              <div style={{ fontFamily:"'Syne',sans-serif", fontSize:28, fontWeight:800, color:parseFloat(media.nota_geral||0)>=7?V.green:V.orange }}>
                {media.nota_geral || '—'}
              </div>
      ...
      <div style={{ background:V.surface, border:`1px solid ${V.border}`, borderRadius:14, overflow:'hidden' }}>
        ...
        {loading ? (
          <div style={{ padding:40, textAlign:'center', color:V.muted }}>Carregando...</div>
        ) : avaliacoes.length === 0 ? (
          <div style={{ padding:48, textAlign:'center', color:V.muted2 }}>
            <div style={{ fontSize:32, marginBottom:10 }}>📋</div>
            <div style={{ fontWeight:500, marginBottom:8 }}>Nenhuma avaliação ainda</div>
            <button onClick={() => setModalOpen(true)}
              style={{ background:V.accent, border:'none', color:'white', padding:'8px 18px', borderRadius:8, cursor:'pointer', fontSize:12 }}>
            ...
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
              ...
              <tbody>
                {avaliacoes.map((a, i) => {
                  ...
                  return (
                    <tr key={a.id} style={{ borderBottom:`1px solid rgba(226,232,244,0.5)` }}
                      onMouseEnter={e=>e.currentTarget.style.background='rgba(26,58,143,0.03)'}
                      onMouseLeave={e=>e.currentTarget.style.background=''}>
                      ...
                      {['comunicacao','trabalho_equipe','iniciativa','lideranca','organizacao'].map(k => (
                        <td key={k} style={{ padding:'12px 14px', textAlign:'center', fontWeight:600,
                          color:parseFloat(a[k]||0)>=8?V.green:parseFloat(a[k]||0)>=6?V.orange:a[k]?V.red:V.muted2 }}>
                          {a[k]??'—'}
                        </td>
                      ))}
                      <td style={{ padding:'12px 14px' }}>
                        <span style={{ fontFamily:"'Syne',sans-serif", fontSize:16, fontWeight:800, color:ng>=7?V.green:ng>=5?V.orange:V.red }}>{a.nota_geral??'—'}</span>
                      </td>
                      <td style={{ padding:'12px 14px', color:V.muted, fontSize:11 }}>{a.avaliador_nome||'—'}</td>
                      <td style={{ padding:'12px 14px' }}>
                        <button onClick={() => handleDelete(a.id)} title="Excluir"
                          style={{ background:'none', border:`1px solid ${V.border}`, color:V.muted2, width:26, height:26, borderRadius:6, cursor:'pointer', fontSize:12, display:'flex', alignItems:'center', justifyContent:'center' }}
...
function ModalCargo({ open, onClose, onSaved, cargo = null }) {
  ...
  const handleSalvar = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const BASE  = process.env.REACT_APP_API_URL || 'https://conectalagoa.onrender.com/api';
      const url    = cargo ? `${BASE}/cargos/${cargo.id}` : `${BASE}/cargos`;
  ...
  return (
    <div onClick={e=>{ if(e.target===e.currentTarget) onClose(); }}
      ...
      <div style={{ background:V.surface, border:`1px solid ${V.border}`, borderRadius:16, padding:28, width:500, maxWidth:'100%' }}>
        <div style={{ fontFamily:"'Syne',sans-serif", fontSize:17, fontWeight:700, marginBottom:20, color:V.text }}>
          {cargo ? 'Editar Cargo' : 'Novo Cargo'}
        </div>
        <div style={{ display:'flex', gap:12, marginBottom:14 }}>
          <div style={{ flex:2 }}>
            <label style={lbl}>Nome do Cargo *</label>
            <input value={form.nome} onChange={set('nome')} placeholder="ex: Analista de Vendas" style={inp}
              onFocus={e=>e.target.style.borderColor=V.accent} onBlur={e=>e.target.style.borderColor=V.border}/>
          </div>
          <div style={{ flex:1 }}>
            <label style={lbl}>Departamento</label>
        ...
        <div style={{ background:V.surface2, borderRadius:10, padding:14, marginBottom:20 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:10 }}>
            {['i','ii','iii','iv','v'].map(f => (
              <div key={f}>
                <label style={{ ...lbl, textAlign:'center', fontSize:10 }}>Faixa {f.toUpperCase()}</label>
                <input type="number" value={form[`faixa_${f}`]} onChange={set(`faixa_${f}`)}
                  placeholder="0,00" style={{ ...inp, textAlign:'center', padding:'8px 6px' }}
                  onFocus={e=>e.target.style.borderColor=V.accent} onBlur={e=>e.target.style.borderColor=V.border}/>
              </div>
            ))}
          </div>
...
// ─── SUBABA: CARGOS E SALÁRIOS ────────────────────────────────────
function SubCargosSalarios({ colaboradores }) {
  ...
  const fetchAll = async () => {
    ...
    try {
      const [cRes, aRes] = await Promise.all([
        fetch(`${BASE}/cargos`,           { headers:{ 'Authorization':`Bearer ${token}` } }),
        fetch(`${BASE}/cargos/aderencia`, { headers:{ 'Authorization':`Bearer ${token}` } }),
      ]);
  ...
  const handleDelete = async (id) => {
    await fetch(`${BASE}/cargos/${id}`, { method:'DELETE', headers:{ 'Authorization':`Bearer ${token}` } });
    fetchAll();
  };

  const fmt = v => v ? `R$ ${parseFloat(v).toLocaleString('pt-BR', { minimumFractionDigits:2 })}` : '—';

  const aderenciaColor = s => s==='aderente'?V.green:s==='abaixo'?V.red:s==='acima'?V.orange:V.muted2;
  const aderenciaLabel = s => s==='aderente'?'✓ Aderente':s==='abaixo'?'↓ Abaixo':s==='acima'?'↑ Acima':'— Sem dados';

  const countAd = { aderente:0, abaixo:0, acima:0, sem_dados:0 };
  aderencia.forEach(a => { countAd[a.status_aderencia] = (countAd[a.status_aderencia]||0)+1; });
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', gap:4, marginBottom:20, borderBottom:`1px solid ${V.border}` }}>
        {[['tabela','📋 Tabela de Cargos'],['aderencia','📊 Aderência Salarial']].map(([id, label]) => (
          <button key={id} onClick={() => setSubTab(id)}
            style={{ padding:'8px 16px', fontSize:12, fontWeight:500, cursor:'pointer', border:'none',
              borderBottom:`2px solid ${subTab===id?V.accent:'transparent'}`,
              color:subTab===id?V.accent:V.muted, background:'none', transition:'all 0.2s',
      ...
      {loading ? (
        <div style={{ textAlign:'center', padding:40, color:V.muted }}>Carregando...</div>
      ) : subTab === 'tabela' ? (
        cargos.length === 0 ? (
          ...
          <div style={{ background:V.surface, border:`1px solid ${V.border}`, borderRadius:14, overflow:'hidden' }}>
            <div style={{ overflowX:'auto' }}>
              <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
                ...
                <tbody>
                  {cargos.map((c, i) => (
                    <tr key={c.id} style={{ borderBottom:`1px solid rgba(226,232,244,0.5)` }}
                      onMouseEnter={e=>e.currentTarget.style.background='rgba(26,58,143,0.03)'}
                      onMouseLeave={e=>e.currentTarget.style.background=''}>
                      ...
                      <td style={{ padding:'12px 14px', textAlign:'center' }}>
                        <span style={{ fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:14 }}>{c.total_colaboradores||0}</span>
                      </td>
                      <td style={{ padding:'12px 14px' }}>
                        <div style={{ display:'flex', gap:6 }}>
                          <button title="Editar" onClick={() => { setEditando(c); setModalOpen(true); }}
                            style={{ background:'none', border:`1px solid ${V.border}`, color:V.muted2, width:28, height:28, borderRadius:6, cursor:'pointer', fontSize:13, display:'flex', alignItems:'center', justifyContent:'center' }}
                            onMouseEnter={e=>{ e.currentTarget.style.borderColor=V.accent; e.currentTarget.style.color=V.accent; }}
                            onMouseLeave={e=>{ e.currentTarget.style.borderColor=V.border; e.currentTarget.style.color=V.muted2; }}>✏</button>
                          <button title="Excluir" onClick={() => handleDelete(c.id)}
                            style={{ background:'none', border:`1px solid ${V.border}`, color:V.muted2, width:28, height:28, borderRadius:6, cursor:'pointer', fontSize:13, display:'flex', alignItems:'center', justifyContent:'center' }}
                            onMouseEnter={e=>{ e.currentTarget.style.borderColor=V.red; e.currentTarget.style.color=V.red; }}
                            onMouseLeave={e=>{ e.currentTarget.style.borderColor=V.border; e.currentTarget.style.color=V.muted2; }}>🗑</button>
                        </div>
        ...
        <div>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:20 }}>
            {[
              { label:'Aderentes', val:countAd.aderente,  color:V.green  },
              { label:'Abaixo',    val:countAd.abaixo,    color:V.red    },
              { label:'Acima',     val:countAd.acima,     color:V.orange },
              { label:'Sem dados', val:countAd.sem_dados, color:V.muted2 },
            ].map((k, i) => (
          ...
          <div style={{ background:V.surface, border:`1px solid ${V.border}`, borderRadius:14, overflow:'hidden' }}>
            {aderencia.length === 0 ? (
              <div style={{ padding:40, textAlign:'center', color:V.muted2, fontSize:13 }}>
                Vincule colaboradores a cargos para ver o relatório de aderência.
              </div>
            ) : (
              <div style={{ overflowX:'auto' }}>
                <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
                  ...
                  <tbody>
                    {aderencia.map((a, i) => {
                      const diff = a.salario_atual && a.salario_referencia
                        ? parseFloat(a.salario_atual) - parseFloat(a.salario_referencia)
                        : null;
                      return (
                        <tr key={a.id} style={{ borderBottom:`1px solid rgba(226,232,244,0.5)` }}
                          onMouseEnter={e=>e.currentTarget.style.background='rgba(26,58,143,0.03)'}
                          ...
                          <td style={{ padding:'12px 14px', textAlign:'center' }}>
                            {a.faixa_atual
                              ? <span style={{ fontSize:10, padding:'3px 9px', borderRadius:20, background:`${V.accent}12`, color:V.accent, fontWeight:700 }}>{a.faixa_atual}</span>
                              : '—'}
                          </td>
                          ...
                          <td style={{ padding:'12px 14px', fontWeight:700, color:diff===null?V.muted2:diff<0?V.red:diff>0?V.orange:V.green }}>
                            {diff===null?'—':`${diff>=0?'+':''}R$ ${diff.toLocaleString('pt-BR',{minimumFractionDigits:2})}`}
                          </td>
                          <td style={{ padding:'12px 14px' }}>
                            <span style={{ fontSize:10, padding:'3px 10px', borderRadius:20, fontWeight:600,
                              background:`${aderenciaColor(a.status_aderencia)}18`,
                              color:aderenciaColor(a.status_aderencia) }}>
                              {aderenciaLabel(a.status_aderencia)}
                            </span>
                          </td>
                        </tr>
                      );
...
// ─── SUBABA: LISTA DE COLABORADORES (antigo PanelColaboradores) ───
function SubListaColaboradores({ colaboradores, onReload }) {
  ...
  const handleExport = () => { window.open(`${BASE}/colaboradores/export-csv?token=${token}`, '_blank'); };

  const handleDelete = async (id) => {
    setDeletandoId(id);
    try {
      await fetch(`${BASE}/colaboradores/${id}`, { method:'DELETE', headers:{ 'Authorization':`Bearer ${token}` } });
      fetchLista();
      onReload && onReload();
    } catch {}
  ...
  return (
    <div>
      ...
      <ModalColaborador
        ...
        onSaved={() => { fetchLista(); onReload && onReload(); }}
        colaborador={editando}
      />
      <ModalImportCSV
        open={modalCSV}
        onClose={() => setModalCSV(false)}
        onDone={() => { fetchLista(); onReload && onReload(); }}
      />
      {desligando && (
        <ModalDesligar
          colaborador={desligando}
          onClose={() => setDesligando(null)}
          onSaved={() => { fetchLista(); setDesligando(null); onReload && onReload(); }}
        />
      )}
    </div>
  );
...
// ─── PAINEL PRINCIPAL COLABORADORES (com 3 subabas) ───────────────
function PanelColaboradores() {
  ...
  const loadColaboradores = () => {
    fetch(`${BASE}/colaboradores?status=ativo&limit=200`, { headers:{ 'Authorization':`Bearer ${token}` } })
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', gap:4, marginBottom:24, borderBottom:`1px solid ${V.border}`, overflowX:'auto' }}>
        {SUB_TABS.map(t => (
          <button key={t.id} onClick={() => setSubTab(t.id)}
            style={{ padding:'10px 22px', fontSize:13, fontWeight:500, cursor:'pointer',
              border:'none', background:'none', whiteSpace:'nowrap',
              borderBottom:`2px solid ${subTab===t.id?V.accent:'transparent'}`,
              color:subTab===t.id?V.accent:V.muted,
      ...
      {subTab==='lista'     && <SubListaColaboradores colaboradores={colaboradores} onReload={loadColaboradores}/>}
      {subTab==='avaliacao' && <SubAvaliacoes colaboradores={colaboradores}/>}
      {subTab==='cargos'    && <SubCargosSalarios colaboradores={colaboradores}/>}
    </div>
  );
}
// ================================================================
